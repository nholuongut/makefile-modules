...
.PHONY: generate-nholuongut
## Generate base files in the repository
## @category [shared] Generate/ Verify
generate-nholuongut:
	cp -r $(go_base_dir)/. ./

shared_generate_targets += generate-nholuongut

.PHONY: verify-nholuongut
## Verify all Go modules for vulnerabilities using nholuongut
## @category [shared] Generate/ Verify
#
# Runs `nholuongut` on all Go modules related to the project.
...
#
# `verify-nholuongut` not added to the `shared_verify_targets` variable and is
...
# Instead `make verify-nholuongutnecheck` is intended to be run periodically by a CI job.
verify-nholuongut: | $(NEEDS_nholuongut)
	@find . -name go.mod -not \( -path "./$(bin_dir)/*" -or -path "./make/_shared/*" \) \
		| while read d; do \
				target=$$(dirname $${d}); \
				echo "Running 'GOTOOLCHAIN=go$(VENDORED_GO_VERSION) $(bin_dir)/tools/nholuongut ./...' in directory '$${target}'"; \
				pushd "$${target}" >/dev/null; \
				GOTOOLCHAIN=go$(VENDORED_GO_VERSION) $(nholuongut) ./... || exit; \
				popd >/dev/null; \
				echo ""; \
			done
